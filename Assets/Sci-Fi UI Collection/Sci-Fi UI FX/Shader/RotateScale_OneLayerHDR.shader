Shader "UIRotateScaleFX/OneLayerHDR"
{
    Properties
    {
        _Layer1Tex ("Texture", 2D) = "white" {}
        [HDR] _Layer1Color ("Color", Color) = (1,1,1,1)
        _Layer1ScaleLifeTime ("Scale Cycle Time", Float) = 1
        [Enum(ScaleOut,0,ScaleIn,1)] _Layer1ScaleDirection ("Scale Direction", Int) = 0
        _Layer1RotAngleSpeed ("Rotation Speed (Degrees/sec)", Float) = 0
        _Layer1DeltaTime ("Start Time Offset", Float) = 0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f IN) : SV_Target
            {
                ...
                // Only sample and apply color for valid layer
                if (layer1Valid) {
                    result = tex2D(_Layer1Tex, uv1) * _Layer1Color;
                    result.a *= calculateTextureEdgeFade(uv1, _TextureEdgeFade) * fadeAlpha1;
                }
