Shader "UIRotateScaleFX/MultiLayerCutCenter"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float calculateCenterFade(float2 uv, float centerFadeFactor)
            {
                ...
                // Invert the calculation compared to edge fade
                // As we get closer to center (smaller distance), we want more transparency
                float centerFade = (distFromCenter * centerFadeFactor / 1.0);
                return saturate(1.0 - saturate(centerFade));
            }
